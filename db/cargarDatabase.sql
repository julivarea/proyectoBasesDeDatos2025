...
INSERT INTO Padrino (dni, nombreYApellido, direccion, codigoPostal, email, facebook, telFijo, telCelular, fechaNacimiento) VALUES
...
('87654321', 'Educación', 1500.00, 'Semanal', 2);

-- Auxiliares para testear
SELECT * FROM Padrino;
SELECT * FROM Donante;
SELECT * FROM Contacto;
SELECT * FROM MedioDePago;
SELECT * FROM TarjetaDeCredito;
SELECT * FROM DebitoTransferencia;
SELECT * FROM Aporta;
SELECT * FROM AuditoriaEliminacionDonante;

-- Pruebas de eliminar cosas
DELETE FROM Donante WHERE dni = '12345678';
DELETE FROM Donante WHERE dni = '87654321';
DELETE FROM Contacto WHERE dni = '11112222';
DELETE FROM MedioDePago WHERE id= 1;
